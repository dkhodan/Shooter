...
void AItemActor::PlayPickUpSound(bool bForcePlaySound)
{
	if (Character && bForcePlaySound)
	{
		if (PickUpSound)
		{
...
{
	// load the data into item rarity data table
	// path to item rarity data table
	FString RarityTablePath(TEXT("DataTable'/Game/Game/DataTable/ItemRarityDataTable.ItemRarityDataTable'"));
	...
	if (RarityTableObject)
	{
		...
		if (RarityRow)
		{
			...
			{
				Mesh->SetCustomDepthStencilValue(RarityRow->CustomerDepthStencil);
			}
		}
	}
...
void AItemActor::PlayEquipSound(bool bForcePlaySound)
{
	if (!Character) return;
...
void AItemActor::StartItemCurve(AShooterCharacter* Char, bool bForcePlaySound)
{
